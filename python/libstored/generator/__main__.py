...
def vhdlinit(o):
    b = (
        lambda: 'x"'
        + (("00" * o.size) + reduce(lambda a, b: "%02x" % b + a, o.encode(o.init, False)))[
            -o.size * 2 :
        ]
        + '"'
    )
    ...
    return {
        "bool": (
            "(7 downto 0 => '0')"
            if o.init == None or b == 'x"00"'
            else "(7 downto 1 => '0', 0 => '1')"
        ),
...
def generate_cmake(libprefix, model_files, output_dir):
    ...
    try:
        libstored_reldir = "${CMAKE_CURRENT_SOURCE_DIR}/" + os.path.relpath(
            libstored_dir, output_dir
        )
    except:
...
def main():
    ...
    parser.add_argument(
        "-p", "--prefix", type=str, help="libstored prefix for cmake library target"
    )
    parser.add_argument(
        "-b", "--big", help="generate for big-endian device (default=little)", action="store_true"
    )
    parser.add_argument(
