...
class StdinLayer(lprot.ProtocolLayer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._stdin_reader = lprot_util.Reader(
            self._from_stdin, thread_name=self.__class__.__name__
        )
        self._reader_task: asyncio.Task | None = asyncio.create_task(
            self._reader_run(), name=self.__class__.__name__
        )
...
class StdioLayer(lprot.ProtocolLayer):
    ...
    def __init__(self, cmd, *args, **kwargs):
        ...
        self._reader: lprot_util.Reader[bytes] = lprot_util.Reader(
            self._from_process, thread_name=f"{self.__class__.__name__}-reader"
        )
        self._writer: lprot_util.Writer[bytes] = lprot_util.Writer(
            self._to_process, thread_name=f"{self.__class__.__name__}-writer"
        )
        self._reader_task: asyncio.Task | None = asyncio.create_task(
            self._reader_run(), name=f"{self.__class__.__name__} reader"
        )
        self._writer_task: asyncio.Task | None = asyncio.create_task(
            self._writer.start(), name=f"{self.__class__.__name__} writer"
        )
    ...
    def set_terminate_callback(
        self, f: Callable[[int], None | Coroutine[None, None, None]]
    ) -> None:
        """
        ...
        self._check_task = asyncio.create_task(
            check_task(), name=f"{self.__class__.__name__} check"
        )
