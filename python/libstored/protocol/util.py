...
class InfiniteStdoutBuffer:
    ...
    def __init__(
        self,
        stdout: typing.TextIO | None = sys.__stdout__,
        cleanup: typing.Callable[[], None] | None = None,
    ):
        self.stdout = stdout
        ...
        self._thread: threading.Thread | None = threading.Thread(
            target=self._worker, daemon=True, name="InfiniteStdoutBufferWorker"
        )
...
class Reader(typing.Generic[T]):
    ...
    async def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._thread_func, daemon=True, name=self._thread_name
        )
        self._thread.start()
...
class Writer(typing.Generic[T]):
    ...
    def __init__(
        self, f: typing.Callable[[T], None], thread_name: str | None = None, *args, **kwargs
    ):
        super().__init__(*args, **kwargs)
