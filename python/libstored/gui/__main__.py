...
class Plotter(laio_tk.Work):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.paused = laio_event.ValueWrapper(
            bool, self._paused_get, self._paused_set, event_name="paused"
        )
...
class ClientConnection(laio_tk.AsyncWidget, ttk.Frame):
    def __init__(
        self,
        app: laio_tk.AsyncApp,
        parent: ttk.Widget,
        client: laio_zmq.ZmqClient,
        ...
        **kwargs,
        ...
        self._host_label.grid(
            row=0, column=0, sticky="e", padx=(0, Style.grid_padding), pady=Style.grid_padding
        )

        self._host = ltk.Entry(self, text=self.host)
        self._host.grid(
            row=0, column=1, sticky="nswe", padx=Style.grid_padding, pady=Style.grid_padding
        )

        self._port_label = ttk.Label(self, text="Port:")
        self._port_label.grid(
            row=0, column=2, sticky="e", padx=Style.grid_padding, pady=Style.grid_padding
        )

        self._port = ltk.Entry(
            self,
            text=str(self.port),
            hint=f"default: {lprot.default_port}",
            validation=r"^[0-9]{0,5}$",
        )
        self._port.grid(
            row=0, column=3, sticky="nswe", padx=Style.grid_padding, pady=Style.grid_padding
        )

        self._multi_var = tk.BooleanVar(value=client.multi)
        self._multi = ttk.Checkbutton(self, text="Multi", variable=self._multi_var)
        self._multi.grid(
            row=0, column=4, sticky="nswe", padx=Style.grid_padding, pady=Style.grid_padding
        )

        self._connect = ttk.Button(self, text="Connect")
        self._connect.grid(
            row=0, column=5, sticky="nswe", padx=(Style.grid_padding, 0), pady=Style.grid_padding
        )
        ...
        if self.client.is_connected():
            ...
            try:
                ...
                self._after_connection(
                    await self.client.identification(), await self.client.version()
                )
...
class ObjectRow(laio_tk.AsyncWidget, ttk.Frame):
    def __init__(
        ...
        **kwargs,
        ...
        self._label.grid(
            row=0, column=0, sticky="w", padx=(0, Style.grid_padding), pady=Style.grid_padding
        )

        self._show_plot = show_plot and Plotter.available and plotter and obj.is_fixed()
        self._plot_var = tk.BooleanVar(value=False)
        self._plot = ttk.Checkbutton(
            self, variable=self._plot_var, command=self._on_plot_check_change
        )
        ...
        self._format.grid(
            row=0, column=2, sticky="nswe", padx=Style.grid_padding, pady=Style.grid_padding
        )
        app.connect(obj.format, self._format.set)

        self._type = ttk.Label(self, text=obj.type_name, width=10, anchor="e")
        self._type.grid(
            row=0, column=3, sticky="e", padx=Style.grid_padding, pady=Style.grid_padding
        )
        ...
        self._poll = ttk.Checkbutton(
            self, variable=self._poll_var, command=self._on_poll_check_change
        )
        self._poll.grid(
            row=0, column=5, sticky="nswe", padx=(Style.grid_padding, 0), pady=Style.grid_padding
        )
        ...
        self._refresh.grid(
            row=0, column=6, sticky="nswe", padx=(Style.grid_padding, 0), pady=Style.grid_padding
        )
        ...
        if x is not None:
            ...
            if self._show_plot:
                self._plot.grid(
                    ...
                    column=1,
                    sticky="nswe",
                    padx=(Style.grid_padding, 0),
                    pady=Style.grid_padding,
                )
        else:
...
class ObjectList(ttk.Frame):
    def __init__(
        ...
        app: GUIClient,
        parent: ttk.Widget,
        objects: list[laio_zmq.Object] = [],
        filter: typing.Callable[[laio_zmq.Object], bool] | None = None,
        show_plot: bool = False,
        *args,
        **kwargs,
    ):
...
class ScrollableFrame(ttk.Frame):
    ...
    def _children(widget: tk.BaseWidget) -> set[tk.BaseWidget]:
        return set(widget.winfo_children()).union(
            *(ScrollableFrame._children(w) for w in widget.winfo_children())
        )
...
class Tools(laio_tk.Work, ttk.Frame):
    def __init__(
        ...
        app: GUIClient,
        parent: ttk.Widget,
        filter_objects: ObjectList,
        refresh_command: typing.Callable[[], typing.Any],
        *args,
        **kwargs,
    ):
        ...
        filter.grid(
            column=0, row=0, sticky="nswe", padx=(0, Style.grid_padding), pady=Style.grid_padding
        )
        ...
        if plotter is not None:
            ...
            self._plot.grid(
                column=1, row=0, sticky="nswe", padx=Style.grid_padding, pady=Style.grid_padding
            )
        ...
        self._default_poll.grid(
            row=0, column=2, sticky="nswe", padx=Style.grid_padding, pady=Style.grid_padding
        )

        refresh_all = ttk.Button(self, text="Refresh all", command=refresh_command)
        refresh_all.grid(
            row=0, column=3, sticky="nswe", padx=(Style.grid_padding, 0), pady=Style.grid_padding
        )
...
class Streams(laio_tk.AsyncWidget, ttk.Frame):
    def __init__(
        self, app: GUIClient, parent: ttk.Widget, client: laio_zmq.ZmqClient, *args, **kwargs
    ):
        super().__init__(app=app, master=parent, *args, **kwargs)
        self._client = client
    ...
    def _on_connected(self):
        if self._client.is_connected():
            self._refresh.grid(
                column=256, row=0, sticky="nswe", padx=(Style.grid_padding * 2, 0), pady=0
            )
            self._on_refresh()
        else:
            self._on_disconnect()
        ...
        for s in streams:
            if s not in self._streams:
                ...
                check = ttk.Checkbutton(
                    self, text=s, command=lambda s=s: self._show_stream(s), variable=var
                )
                check.grid(
                    row=0,
                    column=len(self._streams),
                    sticky="nswe",
                    padx=(Style.grid_padding * 2, 0),
                    pady=0,
                )
...
class GUIClient(laio_tk.AsyncApp):
    def __init__(
        ...
        client: laio_zmq.ZmqClient,
        clear_state: bool = False,
        csv: laio_csv.CsvExport | None = None,
        *args,
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
        ...
        s.configure(
            "Odd.TCheckbutton", background=odd_bg, focuscolor=odd_bg, activebackground=odd_bg
        )
        ...
        s.configure(
            "Even.TCheckbutton", background=even_bg, focuscolor=even_bg, activebackground=even_bg
        )
        ...
        connect.grid(
            ...
            pady=(Style.window_padding, Style.grid_padding),
        )
        ...
        self._tools = Tools(
            app=self, parent=self, filter_objects=self._objects, refresh_command=self._refresh_all
        )
        self._tools.grid(
            column=0, row=1, sticky="nswe", padx=Style.window_padding, pady=Style.grid_padding
        )

        self._scrollable_polled = ScrollableFrame(self)
        self._polled_objects = ObjectList(
            self,
            self._scrollable_polled.content,
            filter=lambda o: o.polling.value is not None,
            show_plot=True,
        )
        ...
        self._manual.grid(
            ...
            sticky="nsew",
            padx=Style.window_padding,
            pady=(Style.grid_padding, Style.window_padding),
        )
    ...
    def _resize_polled_objects(self, *args):
        ...
        if height > 0:
            self._scrollable_polled.grid(
                column=0,
                row=3,
                sticky="nsew",
                padx=Style.window_padding,
                pady=(Style.separator_padding, 0),
            )
            self._scrollable_polled.update_idletasks()
...
def main():
    ...
    parser.add_argument(
        ...
        dest="clear_state",
        default=False,
        help="Clear previously saved state",
        action="store_true",
    )
    parser.add_argument(
        "-D",
        ...
        default=0,
        nargs="?",
        help="Enable deadlock checks after x seconds",
        type=float,
        const=10.0,
    )
    parser.add_argument(
        ...
        + "The file name may include strftime() format codes.",
        const="log.csv",
    )
    parser.add_argument(
        "-e",
        ...
        type=str,
        default=None,
        help="Enable AES-256 CTR encryption with the given pre-shared key file",
        metavar="file",
    )
    ...
    client = laio_zmq.ZmqClient(
        host=args.server, port=args.port, multi=args.multi, use_state="gui", stack=stack
    )
